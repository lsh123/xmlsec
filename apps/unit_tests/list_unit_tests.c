...
test_ptr_list_debug_dump(void) {
    ...
    FILE* xmlOutput = NULL;
    char buffer[256];
    char xmlDumpBuffer[256];

    memset(&list, 0, sizeof(list));
    memset(buffer, 0, sizeof(buffer));
    memset(xmlDumpBuffer, 0, sizeof(xmlDumpBuffer));
    ...
    item1 = test_list_item_create(5);
    item2 = test_list_item_create(6);
    if((item1 == NULL) || (item2 == NULL)) {
        testLog("Error: failed to create test items\n");
        goto done;
    }
    ...
    output = tmpfile();
    xmlOutput = tmpfile();
    if((output == NULL) || (xmlOutput == NULL)) {
        testLog("Error: failed to create temporary output files\n");
        goto done;
    }

    xmlSecPtrListDebugDump(&list, output);
    xmlSecPtrListDebugXmlDump(&list, xmlOutput);

     if((test_list_output_read(output, buffer, sizeof(buffer)) < 0) ||
         (test_list_output_read(xmlOutput, xmlDumpBuffer, sizeof(xmlDumpBuffer)) < 0)) {
        testLog("Error: failed to read debug output\n");
        goto done;
    }
    if((strstr(buffer, "=== list size: 2") == NULL) ||
       (strstr(buffer, "item=5") == NULL) ||
       (strstr(buffer, "item=6") == NULL)) {
        testLog("Error: xmlSecPtrListDebugDump produced unexpected output: %s\n", buffer);
        goto done;
    }
    if((strstr(xmlDumpBuffer, "<List size=\"2\">") == NULL) ||
       (strstr(xmlDumpBuffer, "<Item value=\"5\"/>") == NULL) ||
       (strstr(xmlDumpBuffer, "<Item value=\"6\"/>") == NULL) ||
       (strstr(xmlDumpBuffer, "</List>") == NULL)) {
        testLog("Error: xmlSecPtrListDebugXmlDump produced unexpected output: %s\n", xmlDumpBuffer);
        goto done;
    }
    if((g_listItemDebugDumpCount != 2) || (g_listItemDebugXmlDumpCount != 2)) {
        testLog("Error: debug callbacks were not called for each item\n");
        goto done;
    }
