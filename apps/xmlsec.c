 ...
 */
/**
 * @brief XML Security Library command line utility.
 */
#include <stdlib.h>
#include <string.h>
#include <time.h>
...
static xmlSecAppCmdLineParam enableAsn1SignaturesHackParam = {
    xmlSecAppCmdLineTopicDSigCommon,
...
xmlSecAppExecute(xmlSecAppCommand command, const char** utf8_argv, int argc) {
    ...
    if(xmlSecAppCmdLineParamIsSet(&xxeParam)) {
        /* reset to libxml2's default handler */
    ...
    for(value = concatKdfKeyParam.value; value != NULL; value = value->next) {
        if(value->strValue == NULL) {
            ...
            return(-1);
                    ...
                    (const char*)xmlSecNameConcatKdf, value->strValue, value->paramNameValue) < 0) {
    ...
    for(value = pbkdf2KeyParam.value; value != NULL; value = value->next) {
        if(value->strValue == NULL) {
            ...
            return(-1);
                    ...
                    (const char*)xmlSecNamePbkdf2, value->strValue, value->paramNameValue) < 0) {
    ...
    for(value = hkdfKeyParam.value; value != NULL; value = value->next) {
        if(value->strValue == NULL) {
            ...
            return(-1);
                    ...
                    (const char*)xmlSecNameHkdf, value->strValue, value->paramNameValue) < 0) {
