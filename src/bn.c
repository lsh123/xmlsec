...
xmlSecBnFromString(xmlSecBnPtr bn, const xmlChar* str, xmlSecSize base) {
    ...
    if(ret < 0) {
        xmlSecInternalError2(NULL, "xmlSecBnRevLookupTable", "size=%d", len / 2 + 1);
        return (-1);
    }
    ...
    while(i < len) {
        ...
        if(ret < 0) {
            xmlSecInternalError2(NULL, "xmlSecBnMul", "base=%d", base);
            return (-1);
        }

        ret = xmlSecBnAdd(bn, nn);
        if(ret < 0) {
            xmlSecInternalError2(NULL, "xmlSecBnAdd", "base=%d", base);
            return (-1);
        }
    }
    ...
    if(((size > 0) && (data[0] > 127)) || (size == 0))  {
        ...
        if(ret < 0) {
            xmlSecInternalError2(NULL, "xmlSecBufferPrepend", "base=%d", base);
            return (-1);
        }
    }
...
xmlSecBnToString(xmlSecBnPtr bn, xmlSecSize base) {
    ...
    if(ret < 0) {
        xmlSecInternalError2(NULL, "xmlSecBnCreate", "size=%d", size);
        return (NULL);
    }

    ret = xmlSecBnSetData(&bn2, data, size);
    if(ret < 0) {
        xmlSecInternalError2(NULL, "xmlSecBnSetData", "size=%d", size);
    ...
    if((size > 0) && (data[0] > 127)) {
        ...
        if(ret < 0) {
            xmlSecInternalError2(NULL, "xmlSecBnAdd", "size=%d", size);
            xmlSecBnFinalize(&bn2);
            return (NULL);
        }
        for(i = 0; i < size; ++i) {
    ...
    for(i = 0; (xmlSecBufferGetSize(&bn2) > 0) && (i < len); i++) {
        if(xmlSecBnDiv(&bn2, base, &nn) < 0) {
            xmlSecInternalError2(NULL, "xmlSecBnDiv", "base=%d", base);
            xmlFree(res);
            xmlSecBnFinalize(&bn2);
            return (NULL);
        }
...
xmlSecBnMul(xmlSecBnPtr bn, int multiplier) {
    ...
    while(over > 0) {
        ...
        ret = xmlSecBufferPrepend(bn, &ch, 1);
        if(ret < 0) {
            xmlSecInternalError2(NULL, "xmlSecBufferPrepend", "size=%d", 1);
            return (-1);
        }
    }
...
xmlSecBnDiv(xmlSecBnPtr bn, int divider, int* mod) {
    ...
    if(i > 0) {
        ...
        if(ret < 0) {
            xmlSecInternalError2(NULL, "xmlSecBufferRemoveHead", "size=%d", i);
            return (-1);
        }
    }
    return(0);
...
xmlSecBnGetNodeValue(xmlSecBnPtr bn, xmlNodePtr cur, xmlSecBnFormat format, int reverse) {
    ...
    case xmlSecBnHex:
        ...
        if(ret < 0) {
            xmlSecInternalError(NULL, "xmlSecBnFromHexString");
            xmlFree(content);
            return(-1);
        }
        xmlFree(content);
    ...
    case xmlSecBnDec:
        ...
        if(ret < 0) {
            xmlSecInternalError(NULL, "xmlSecBnFromDecString");
            xmlFree(content);
            return(-1);
        }
        xmlFree(content);
...
xmlSecBnSetNodeValue(xmlSecBnPtr bn, xmlNodePtr cur, xmlSecBnFormat format, int reverse, int addLineBreaks) {
    ...
    case xmlSecBnHex:
        ...
        if(content == NULL) {
            xmlSecInternalError(NULL, "xmlSecBnToHexString");
            xmlFree(content);
            return(-1);
        }
    ...
    case xmlSecBnDec:
        ...
        if(content == NULL) {
            xmlSecInternalError(NULL, "xmlSecBnToDecString");
            xmlFree(content);
            return(-1);
        }
        xmlNodeSetContent(cur, content);
