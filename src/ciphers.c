 ...
 */
int  	
xmlSecCipherTransformRead(xmlSecTransformPtr transform, 
    ...
    if((cipher->iv != NULL) && (cipher->ivPos < cipherId->ivSize)) {
	if(cipher->encode) {
	    if(cipher->ivPos == 0) {
	        /* generate random iv */
		ret = RAND_bytes(cipher->iv, cipherId->ivSize);
		if(ret != 1) {
	    ...
	    if(size > cipherId->ivSize - cipher->ivPos) {
		size = cipherId->ivSize - cipher->ivPos; 
	    }
	    memcpy(buf, cipher->iv + cipher->ivPos, size);
	    cipher->ivPos += size;
	    if(cipher->ivPos >= cipherId->ivSize) {
	    ...
	    while(cipherId->ivSize > cipher->ivPos) {
		ret = xmlSecTransformReadBin((xmlSecTransformPtr)cipher->prev,
			    cipher->iv + cipher->ivPos, 
			    cipherId->ivSize - cipher->ivPos);
		if(ret < 0) {
		    xmlSecError(XMLSEC_ERRORS_HERE,
    ...
    while(res + cipherId->bufOutSize <= size) {
	ret = xmlSecTransformReadBin((xmlSecTransformPtr)cipher->prev, 
				     cipher->bufIn, cipherId->bufInSize);	
	if(ret < 0) {
	    xmlSecError(XMLSEC_ERRORS_HERE,
			XMLSEC_ERRORS_R_XMLSEC_FAILED,
			"xmlSecTransformRead - %d", ret);
 ...
 */
int  	
xmlSecCipherTransformWrite(xmlSecTransformPtr transform, 
	...
	if(cipher->encode) {
	    ...
	    ret = xmlSecTransformWriteBin((xmlSecTransformPtr)cipher->next, 
					    cipher->iv, cipherId->ivSize);
	    if(ret < 0) {
		xmlSecError(XMLSEC_ERRORS_HERE,
			    XMLSEC_ERRORS_R_XMLSEC_FAILED,
		...
		s = cipherId->ivSize - cipher->ivPos;
	    }
    ...
    while(res < size) {
	block = (size - res);
	if(block > cipherId->bufInSize) {
	    block = cipherId->bufInSize;
	}
 ...
 */
int
xmlSecCipherTransformFlush(xmlSecTransformPtr transform) {
    xmlSecCipherTransformPtr cipher;
    xmlSecCipherTransformId cipherId;
    int ret;
    ...
    ret = xmlSecTransformFlushBin((xmlSecTransformPtr)cipher->next);
    if(ret < 0){
	xmlSecError(XMLSEC_ERRORS_HERE,
		    XMLSEC_ERRORS_R_XMLSEC_FAILED,
...
xmlSecEvpCipherFinal(xmlSecCipherTransformPtr cipher) {
	...
	if(ret == 1) {
	    ...
	    if(b > 1) {
		xmlSecAssert2(b <= (int)sizeof(ctx->final), -1);
		xmlSecAssert2(b <= (int)cipherId->bufOutSize, -1);
		res = b - ctx->final[b - 1];
		if(res > 0) {
...
xmlSecCipherUpdate(xmlSecTransformPtr transform,
    ...
    if(((cipherId->cipherUpdate) != NULL) && (size > 0)) {
	return((cipherId->cipherUpdate)(cipher, (unsigned char *)buffer, size)); 
    }
    return(0);    
}
...
xmlSecCipherFinal(xmlSecTransformPtr transform) {
    ...
    cipher = (xmlSecCipherTransformPtr)transform;
    cipherId = (xmlSecCipherTransformId)transform->id;
    if((cipherId->cipherFinal) != NULL) {
	return((cipherId->cipherFinal)(cipher)); 
    }
    return(0);    
}
