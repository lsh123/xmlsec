...
xmlSecGnuTLSKeyDataDsaAdoptPrivateKey(xmlSecKeyDataPtr data, gnutls_x509_privkey_t dsa_key) {
    ...
    if(ret < 0) {
        xmlSecInternalError(NULL, "xmlSecGnuTLSConvertParamsToMpis");
    ...
    if(ret < 0) {
        xmlSecInternalError(NULL, "xmlSecGCryptKeyDataDsaAdoptKeyPair");
...
xmlSecGnuTLSKeyDataRsaAdoptPrivateKey(xmlSecKeyDataPtr data, gnutls_x509_privkey_t rsa_key) {
    ...
    if(ret < 0) {
        xmlSecInternalError(NULL, "xmlSecGCryptKeyDataRsaAdoptKeyPair");
        gcry_sexp_release(pub_key);
        gcry_sexp_release(priv_key);
        return(-1);
    }
