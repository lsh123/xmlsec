 ...
 * @brief HMAC transforms implementation for GnuTLS.
 */
...
xmlSecGnuTLSHmacInitialize(xmlSecTransformPtr transform) {
    ...
    if(hmacSize == 0) {
        xmlSecInternalError2("gnutls_hmac_get_len", xmlSecTransformGetName(transform), "hmacSize=" XMLSEC_SIZE_FMT, hmacSize);
        return(-1);
    }
...
xmlSecGnuTLSHmacSetKey(xmlSecTransformPtr transform, xmlSecKeyPtr key) {
    ...
    xmlSecAssert2(xmlSecBufferGetData(keyBuf) != NULL, -1);
...
#define XMLSEC_GNUTLS_HMAC_KLASS_EX(name, readNode)                                             \
static xmlSecTransformKlass xmlSecGnuTLSHmac ## name ## Klass = {                               \
...
#define XMLSEC_GNUTLS_HMAC_KLASS(name)                                                          \
    XMLSEC_GNUTLS_HMAC_KLASS_EX(name, xmlSecGnuTLSHmacNodeRead)
