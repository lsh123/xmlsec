...
xmlSecMSCngKeyDataCertGetDsaPubkey(PCERT_PUBLIC_KEY_INFO spki, BCRYPT_KEY_HANDLE* key) {
    ...
    xmlSecAssert2(spki->PublicKey.cbData != 0, -1);
    xmlSecAssert2(spki->PublicKey.pbData != NULL, -1);
    ...
    if((pSize <= XMLSEC_MSCNG_DSA_V1_MAX_P_SIZE) && (qBlobSize != XMLSEC_MSCNG_DSA_MAX_Q_SIZE)) {
        xmlSecInvalidDataError("DSA key with p <= 1024 bits and q > 20 bytes is not supported", NULL);
        goto done;
    }
    if((pSize <= XMLSEC_MSCNG_DSA_V1_MAX_P_SIZE) && (qBlobSize == XMLSEC_MSCNG_DSA_MAX_Q_SIZE)) {
...
xmlSecMSCngIsDsaBcryptKey(BCRYPT_KEY_HANDLE hKey) {
    ...
    return (((wcslen(algName) == wcslen(BCRYPT_DSA_ALGORITHM)) && (wcsncmp(algName, BCRYPT_DSA_ALGORITHM, wcslen(BCRYPT_DSA_ALGORITHM)) == 0)) ? 1 : 0);
}
...
xmlSecMSCngKeyDataDsaPubkeyWrite(BCRYPT_KEY_HANDLE pubkey, xmlSecKeyValueDsaPtr dsaValue) {
    ...
    if(dsakey->dwMagic == BCRYPT_DSA_PUBLIC_MAGIC) {
        ...
        if (ret < 0) {
            xmlSecInternalError2("xmlSecBufferSetData(q)", NULL, "qLen=%lu", (unsigned long)sizeof(dsakey->q));
            goto done;
        }
