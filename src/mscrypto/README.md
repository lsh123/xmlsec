...
Store. If keys are not found in the XMLSEC Simple Keys Store, then the MS Certificate store is
used to lookup keys. The certificate store is only used on a read-only basis, so it is
not possible to store keys via the keys store into the MS certificate store.

When the xmlsec application is started, with the `--crypto-config` parameter the name of
the (system) keystore can be given. That keystore will be used for certificates
and keys lookup. For the key name, two types of values can be given:
...
1) Default keys manager doesn't use trusted certs in MS Crypto Store (also see
