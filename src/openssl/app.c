...
xmlSecOpenSSLAppInit(const char* config) {
    ...
    ret = OPENSSL_init_crypto(OPENSSL_INIT_LOAD_CRYPTO_STRINGS | 
                              ...
                              NULL);
    if(ret != 1) {
        xmlSec2OpenSSLError("OPENSSL_init_crypto", NULL);
        return(-1);
    }
...
xmlSecOpenSSLAppKeyLoad(const char *filename, xmlSecKeyDataFormat format,
    ...
    if(bio == NULL) {
        xmlSec2OpenSSLError("BIO_new_file", filename);
        return(NULL);
    }
...
xmlSecOpenSSLAppKeyLoadMemory(const xmlSecByte* data, xmlSecSize dataSize,
    ...
    if(bio == NULL) {
        xmlSec2OpenSSLError("BIO_new_mem_buf", NULL);
        return(NULL);
    }
...
xmlSecOpenSSLAppKeyLoadBIO(BIO* bio, xmlSecKeyDataFormat format,
    ...
    case xmlSecKeyDataFormatPem:
        ...
        if(pKey == NULL) {
            ...
            pKey = PEM_read_bio_PUBKEY(bio, NULL,
                            ...
                            pwdCallbackCtx);
            if(pKey == NULL) {
                xmlSec2OpenSSLError("PEM_read_bio_PrivateKey and PEM_read_bio_PUBKEY", NULL);
                return(NULL);
            }
        }
        break;
    case xmlSecKeyDataFormatDer:
        ...
        if(pKey == NULL) {
            ...
            if(pKey == NULL) {
                xmlSec2OpenSSLError("d2i_PrivateKey_bio and d2i_PUBKEY_bio", NULL);
                return(NULL);
            }
        }
        break;
    case xmlSecKeyDataFormatPkcs8Pem:
        ...
        pKey = PEM_read_bio_PrivateKey(bio, NULL,
                            ...
                            pwdCallbackCtx);
        if(pKey == NULL) {
            xmlSec2OpenSSLError("PEM_read_bio_PrivateKey", NULL);
            return(NULL);
        }
        break;
...
xmlSecOpenSSLAppPkcs12LoadBIO(BIO* bio, const char *pwd,
    ...
    if(p12 == NULL) {
        xmlSec2OpenSSLError("d2i_PKCS12_fp", NULL);
        goto done;
    }

    ret = PKCS12_verify_mac(p12, pwd, (pwd != NULL) ? strlen(pwd) : 0);
    if(ret != 1) {
        xmlSec2OpenSSLError("PKCS12_verify_mac", NULL);
        goto done;
    }

    ret = PKCS12_parse(p12, pwd, &pKey, &cert, &chain);
    if(ret < 0) {
        xmlSec2OpenSSLError("PKCS12_parse", NULL);
        goto done;
    }
    ...
    if(chain == NULL) {
        ...
        if(chain == NULL) {
            xmlSec2OpenSSLError("sk_X509_new_null", NULL);
            goto done;
        }
    }
    ...
    */
    ...
    if(has_cert == 0) {
        ...
        if(tmpcert == NULL) {
            xmlSec2OpenSSLError("X509_dup", xmlSecKeyDataGetName(x509Data));
            goto done;
        }

        ret = sk_X509_push(chain, tmpcert);
        if(ret < 1) {
            xmlSec2OpenSSLError("sk_X509_push", xmlSecKeyDataGetName(x509Data));
            X509_free(tmpcert);
            goto done;
        }
    }
...
xmlSecOpenSSLAppCertLoadBIO(BIO* bio, xmlSecKeyDataFormat format) {
    ...
    case xmlSecKeyDataFormatCertPem:
        ...
        if(cert == NULL) {
            xmlSec2OpenSSLError("PEM_read_bio_X509_AUX", NULL);
            return(NULL);
        }
        break;
    case xmlSecKeyDataFormatDer:
...
xmlSecOpenSSLAppLoadRANDFile(const char *filename) {
    ...
    if((filename == NULL) || !RAND_load_file(filename, -1)) {
        if(RAND_status() == 0) {
            xmlSec2OpenSSLError("RAND_load_file", filename);
            return 0;
        }
    }
    seeded = 1;
...
xmlSecOpenSSLAppSaveRANDFile(const char *filename) {
    ...
    if((filename == NULL) || !RAND_write_file(filename)) {
        xmlSec2OpenSSLError("RAND_write_file", filename);
        return 0;
    }
...
xmlSecOpenSSLDefaultPasswordCallback(char *buf, int bufsize, int verify, void *userdata) {
    ...
    for(i = 0; i < 3; i++) {
        ...
        if(ret != 0) {
            xmlSec2OpenSSLError("EVP_read_pw_string", NULL);
            return(-1);
        }
