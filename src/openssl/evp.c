...
xmlSecOpenSSLKeyDataDsaGenerate(xmlSecKeyDataPtr data, xmlSecSize sizeBits, xmlSecKeyDataType type ATTRIBUTE_UNUSED) {
    ...
    if(ret != 1) {
        xmlSecOpenSSLError("DSA_generate_parameters_ex", xmlSecKeyDataGetName(data));
        DSA_free(dsa);
        return(-1);
    }
...
xmlSecOpenSSLKeyDataEcdsaAdoptEcdsa(xmlSecKeyDataPtr data, EC_KEY* ecdsa) {
    ...
    if(ecdsa != NULL) {
        ...
        if(ret != 1) {
            xmlSecOpenSSLError("EVP_PKEY_assign_EC_KEY", xmlSecKeyDataGetName(data));
            EVP_PKEY_free(pKey);
            return(-1);
        }
    }
...
xmlSecOpenSSLKeyDataRsaGenerate(xmlSecKeyDataPtr data, xmlSecSize sizeBits, xmlSecKeyDataType type ATTRIBUTE_UNUSED) {
    ...
    if(ret != 1) {
        xmlSecOpenSSLError("RSA_generate_key", xmlSecKeyDataGetName(data));
        RSA_free(rsa);
        BN_free(e);
        return(-1);
    }
