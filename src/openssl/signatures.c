...
xmlSecOpenSSLSignatureInitialize(xmlSecTransformPtr transform) {
    ...
    if(ret != 1) {
        xmlSec2OpenSSLError("EVP_DigestInit", xmlSecTransformGetName(transform));
        return(-1);
    }
...
xmlSecOpenSSLSignatureExecute(xmlSecTransformPtr transform, int last, xmlSecTransformCtxPtr transformCtx) {
    ...
    if((transform->status == xmlSecTransformStatusWorking) && (inSize > 0)) {
        ...
        if(ret != 1) {
            xmlSec2OpenSSLError("EVP_DigestUpdate", xmlSecTransformGetName(transform));
            return(-1);
        }
    ...
    if((transform->status == xmlSecTransformStatusWorking) && (last != 0)) {
        ...
        if(ret != 1) {
            xmlSec2OpenSSLError("EVP_DigestFinal", xmlSecTransformGetName(transform));
            return(-1);
        }
        xmlSecAssert2(ctx->dgstSize > 0, -1);
...
xmlSecOpenSSLSignatureDsaSign(xmlSecOpenSSLSignatureCtxPtr ctx, xmlSecBufferPtr out) {
    ...
    if(dsaKey == NULL) {
        xmlSec2OpenSSLError("EVP_PKEY_get1_DSA", NULL);
        goto done;
    }
    ...
    if(sig == NULL) {
        xmlSec2OpenSSLError("DSA_do_sign", NULL);
        goto done;
    }
    ...
    if((rr == NULL) || (ss == NULL)) {
        xmlSec2OpenSSLError("DSA_SIG_get0", NULL);
        goto done;
    }
...
xmlSecOpenSSLSignatureDsaVerify(xmlSecOpenSSLSignatureCtxPtr ctx, const xmlSecByte* signData, xmlSecSize signSize) {
    ...
    sig = DSA_SIG_new();
    if (sig == NULL) {
        xmlSec2OpenSSLError("DSA_SIG_new", NULL);
        goto done;
    }

    rr = BN_bin2bn(signData, signHalfSize, NULL);
    if(rr == NULL) {
        xmlSec2OpenSSLError("BN_bin2bn(sig->r)", NULL);
        goto done;
    }
    ss = BN_bin2bn(signData + signHalfSize, signHalfSize, NULL);
    if(ss == NULL) {
        xmlSec2OpenSSLError("BN_bin2bn(sig->s)", NULL);
        goto done;
    }

    ret = DSA_SIG_set0(sig, rr, ss);
    if(ret == 0) {
        xmlSec2OpenSSLError("DSA_SIG_set0", NULL);
        goto done;
    }
    ...
    if(ret < 0) {
        xmlSec2OpenSSLError("DSA_do_verify", NULL);
        goto done;
    }
...
xmlSecOpenSSLSignatureEcdsaSign(xmlSecOpenSSLSignatureCtxPtr ctx, xmlSecBufferPtr out) {
    ...
    if(sig == NULL) {
        xmlSec2OpenSSLError("ECDSA_do_sign", NULL);
        goto done;
    }
    ...
    if((rr == NULL) || (ss == NULL)) {
        xmlSec2OpenSSLError("ECDSA_SIG_get0", NULL);
        goto done;
    }
...
xmlSecOpenSSLSignatureEcdsaVerify(xmlSecOpenSSLSignatureCtxPtr ctx, const xmlSecByte* signData, xmlSecSize signSize) {
    ...
    ecKey = EVP_PKEY_get1_EC_KEY(ctx->pKey);
    if(ecKey == NULL) {
        xmlSec2OpenSSLError("EVP_PKEY_get1_EC_KEY", NULL);
        goto done;
    }
    ...
    if(ret == 0) {
        xmlSec2OpenSSLError("ECDSA_SIG_set0()", NULL);
        goto done;
    }
    rr = NULL;
    ss = NULL;
    ...
    if(ret < 0) {
        xmlSec2OpenSSLError("ECDSA_do_verify", NULL);
        goto done;
    }
