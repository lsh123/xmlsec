...
xmlSecOpenSSLKeyDataX509AdoptCert(xmlSecKeyDataPtr data, X509* cert) {
    ...
    if(ctx->certsList == NULL) {
        ...
        if(ctx->certsList == NULL) {
            xmlSecOpenSSLError(xmlSecKeyDataGetName(data), "sk_X509_new_null");
            return(-1);
        }
    }
    ...
    if(ret < 1) {
        xmlSecOpenSSLError(xmlSecKeyDataGetName(data), "sk_X509_push");
        return(-1);
    }
...
xmlSecOpenSSLKeyDataX509AdoptCrl(xmlSecKeyDataPtr data, X509_CRL* crl) {
    ...
    if(ctx->crlsList == NULL) {
        ...
        if(ctx->crlsList == NULL) {
            xmlSecOpenSSLError(xmlSecKeyDataGetName(data), "sk_X509_CRL_new_null");
            return(-1);
        }
    }
    ...
    if(ret < 1) {
        xmlSecOpenSSLError(xmlSecKeyDataGetName(data), "sk_X509_CRL_push");
        return(-1);
    }
...
xmlSecOpenSSLKeyDataX509Duplicate(xmlSecKeyDataPtr dst, xmlSecKeyDataPtr src) {
    ...
    for(pos = 0; pos < size; ++pos) {
        ...
        if(certDst == NULL) {
            xmlSecOpenSSLError(xmlSecKeyDataGetName(dst), "X509_dup");
            return(-1);
        }

        ret = xmlSecOpenSSLKeyDataX509AdoptCert(dst, certDst);
        if(ret < 0) {
            xmlSecInternalError(xmlSecKeyDataGetName(dst), "xmlSecOpenSSLKeyDataX509AdoptCert");
            X509_free(certDst);
            return(-1);
        }
    }
    ...
    for(pos = 0; pos < size; ++pos) {
        ...
        if(crlDst == NULL) {
            xmlSecOpenSSLError(xmlSecKeyDataGetName(dst), "X509_CRL_dup");
            return(-1);
        }

        ret = xmlSecOpenSSLKeyDataX509AdoptCrl(dst, crlDst);
        if(ret < 0) {
            xmlSecInternalError(xmlSecKeyDataGetName(dst), "xmlSecOpenSSLKeyDataX509AdoptCrl");
    ...
    if(certSrc != NULL) {
        ...
        if(ret < 0) {
            xmlSecInternalError(xmlSecKeyDataGetName(dst), "xmlSecOpenSSLKeyDataX509AdoptKeyCert");
            X509_free(certDst);
            return(-1);
        }
    }
...
xmlSecOpenSSLKeyDataX509XmlRead(xmlSecKeyDataId id, xmlSecKeyPtr key,
    ...
    if(data == NULL) {
        xmlSecInternalError(xmlSecKeyDataKlassGetName(id), "xmlSecKeyEnsureData");
        return(-1);
    }

    ret = xmlSecOpenSSLX509DataNodeRead(data, node, keyInfoCtx);
    if(ret < 0) {
        xmlSecInternalError(xmlSecKeyDataKlassGetName(id), "xmlSecOpenSSLX509DataNodeRead");
        return(-1);
    }
    ...
    if((keyInfoCtx->flags & XMLSEC_KEYINFO_FLAGS_X509DATA_DONT_VERIFY_CERTS) == 0) {
        ...
        if(ret < 0) {
            xmlSecInternalError(xmlSecKeyDataKlassGetName(id), "xmlSecOpenSSLKeyDataX509VerifyAndExtractKey");
            return(-1);
        }
    }
    return(0);
...
xmlSecOpenSSLX509CertificateNodeRead(xmlSecKeyDataPtr data, xmlNodePtr node, xmlSecKeyInfoCtxPtr keyInfoCtx) {
    ...
    if(cert == NULL) {
        xmlSecInternalError(xmlSecKeyDataGetName(data), "xmlSecOpenSSLX509CertBase64DerRead");
        xmlFree(content);
        return(-1);
    }
    ...
    if(ret < 0) {
        xmlSecInternalError(xmlSecKeyDataGetName(data), "xmlSecOpenSSLKeyDataX509AdoptCert");
        X509_free(cert);
        xmlFree(content);
        return(-1);
    }
...
xmlSecOpenSSLX509CertificateNodeWrite(X509* cert, xmlNodePtr node, xmlSecKeyInfoCtxPtr keyInfoCtx) {
    ...
    if(buf == NULL) {
        xmlSecInternalError(NULL, "xmlSecOpenSSLX509CertBase64DerWrite");
        return(-1);
    }
...
xmlSecOpenSSLX509SubjectNameNodeRead(xmlSecKeyDataPtr data, xmlNodePtr node, xmlSecKeyInfoCtxPtr keyInfoCtx) {
    ...
    if(x509Store == NULL) {
        xmlSecInternalError(xmlSecKeyDataGetName(data), "xmlSecKeysMngrGetDataStore");
        return(-1);
    }
    ...
    cert2 = X509_dup(cert);
    if(cert2 == NULL) {
        xmlSecOpenSSLError(xmlSecKeyDataGetName(data), "X509_dup");
        xmlFree(subject);
        return(-1);
    }
...
xmlSecOpenSSLX509SubjectNameNodeWrite(X509* cert, xmlNodePtr node, xmlSecKeyInfoCtxPtr keyInfoCtx ATTRIBUTE_UNUSED) {
    ...
    if(buf == NULL) {
        xmlSecInternalError(NULL, "xmlSecOpenSSLX509NameWrite(X509_get_subject_name)");
        return(-1);
    }
...
xmlSecOpenSSLX509IssuerSerialNodeWrite(X509* cert, xmlNodePtr node, xmlSecKeyInfoCtxPtr keyInfoCtx ATTRIBUTE_UNUSED) {
    ...
    if(buf == NULL) {
        xmlSecInternalError(NULL, "xmlSecOpenSSLX509NameWrite(X509_get_issuer_name)");
        return(-1);
    }
    ...
    if(buf == NULL) {
        xmlSecInternalError(NULL, "xmlSecOpenSSLASN1IntegerWrite(X509_get_serialNumber)");
        return(-1);
    }
...
xmlSecOpenSSLX509SKINodeWrite(X509* cert, xmlNodePtr node, xmlSecKeyInfoCtxPtr keyInfoCtx ATTRIBUTE_UNUSED) {
    ...
    if(buf == NULL) {
        xmlSecInternalError(NULL, "xmlSecOpenSSLX509SKIWrite");
        return(-1);
    }
...
xmlSecOpenSSLX509CRLNodeRead(xmlSecKeyDataPtr data, xmlNodePtr node, xmlSecKeyInfoCtxPtr keyInfoCtx) {
    ...
    if(crl == NULL) {
        xmlSecInternalError(xmlSecKeyDataGetName(data), "xmlSecOpenSSLX509CrlBase64DerRead");
        xmlFree(content);
        return(-1);
    }
    ...
    if(ret < 0) {
        xmlSecInternalError(xmlSecKeyDataGetName(data), "xmlSecOpenSSLKeyDataX509AdoptCrl");
        X509_CRL_free(crl);
        xmlFree(content);
        return(-1);
    }
...
xmlSecOpenSSLX509CRLNodeWrite(X509_CRL* crl, xmlNodePtr node, xmlSecKeyInfoCtxPtr keyInfoCtx) {
    ...
    if(buf == NULL) {
        xmlSecInternalError(NULL, "xmlSecOpenSSLX509CrlBase64DerWrite");
        return(-1);
    }
...
xmlSecOpenSSLKeyDataX509VerifyAndExtractKey(xmlSecKeyDataPtr data, xmlSecKeyPtr key,
    ...
    if((ctx->keyCert == NULL) && (ctx->certsList != NULL) && (xmlSecKeyGetValue(key) == NULL)) {
        ...
        if(cert != NULL) {
            ...
            if(keyValue == NULL) {
                xmlSecInternalError(xmlSecKeyDataGetName(data), "xmlSecOpenSSLX509CertGetKey");
                return(-1);
            }

            /* verify that the key matches our expectations */
            if(xmlSecKeyReqMatchKeyValue(&(keyInfoCtx->keyReq), keyValue) != 1) {
                xmlSecInternalError(xmlSecKeyDataGetName(data), "xmlSecKeyReqMatchKeyValue");
...
xmlSecOpenSSLX509CertGetTime(const ASN1_TIME* t, time_t* res) {
    ...
    if(!ASN1_TIME_check(t)) {
        xmlSecOpenSSLError(NULL, "ASN1_TIME_check");
        return(-1);
    }
...
xmlSecOpenSSLX509CertGetKey(X509* cert) {
    ...
    if(data == NULL) {
        xmlSecInternalError(NULL, "xmlSecOpenSSLEvpKeyAdopt");
        EVP_PKEY_free(pKey);
        return(NULL);
    }
...
xmlSecOpenSSLX509CertDerRead(const xmlSecByte* buf, xmlSecSize size) {
    ...
    if(mem == NULL) {
        xmlSecOpenSSLError(NULL, "BIO_new");
        return(NULL);
    }

    ret = BIO_write(mem, buf, size);
    if(ret <= 0) {
        xmlSecOpenSSLError(NULL, "BIO_write");
        BIO_free_all(mem);
        return(NULL);
    }
...
xmlSecOpenSSLX509CertBase64DerWrite(X509* cert, int base64LineWrap) {
    ...
    if((size <= 0) || (p == NULL)){
        xmlSecOpenSSLError(NULL, "BIO_get_mem_data");
        BIO_free_all(mem);
        return(NULL);
    }
...
xmlSecOpenSSLX509CrlDerRead(xmlSecByte* buf, xmlSecSize size) {
    ...
    if(crl == NULL) {
        xmlSecOpenSSLError(NULL, "d2i_X509_CRL_bio");
        BIO_free_all(mem);
        return(NULL);
    }
...
xmlSecOpenSSLX509NameWrite(X509_NAME* nm) {
    ...
    if (X509_NAME_print_ex(mem, nm, 0, XN_FLAG_RFC2253) <=0) {
        xmlSecOpenSSLError(NULL, "X509_NAME_print_ex");
        BIO_free_all(mem);
        return(NULL);
    }
...
xmlSecOpenSSLASN1IntegerWrite(ASN1_INTEGER *asni) {
    ...
    if(bn == NULL) {
        xmlSecOpenSSLError(NULL, "ASN1_INTEGER_to_BN");
        return(NULL);
    }

    p = BN_bn2dec(bn);
    if (p == NULL) {
        xmlSecOpenSSLError(NULL, "BN_bn2dec");
        BN_free(bn);
        return(NULL);
    }
    BN_free(bn);
...
xmlSecOpenSSLX509SKIWrite(X509* cert) {
    ...
    if (index < 0) {
        xmlSecOpenSSLError(NULL, "X509_get_ext_by_NID(): Certificate without SubjectKeyIdentifier extension");
        return(NULL);
    }

    ext = X509_get_ext(cert, index);
    if (ext == NULL) {
        xmlSecOpenSSLError(NULL, "X509_get_ext");
        return(NULL);
    }

    keyId = X509V3_EXT_d2i(ext);
    if (keyId == NULL) {
        xmlSecOpenSSLError(NULL, "X509V3_EXT_d2i");
...
xmlSecOpenSSLKeyDataRawX509CertBinRead(xmlSecKeyDataId id, xmlSecKeyPtr key,
    ...
    if(cert == NULL) {
        xmlSecInternalError(NULL, "xmlSecOpenSSLX509CertDerRead");
        return(-1);
    }
    ...
    ret = xmlSecOpenSSLKeyDataX509AdoptCert(data, cert);
    if(ret < 0) {
        xmlSecInternalError(xmlSecKeyDataKlassGetName(id), "xmlSecOpenSSLKeyDataX509AdoptCert");
        X509_free(cert);
        return(-1);
    }
