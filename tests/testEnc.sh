...
##########################################################################
#
# NIST aes gcm decryption
#
##########################################################################
if [ $usingWSL -eq 0 ] ; then
    nistFolderRoot=$topfolder/nist-aesgcm/decrypt
else
    nistFolderRoot=`wslpath $topfolder/nist-aesgcm/decrypt`
fi
for nistfolder in $nistFolderRoot/*
do
    transformName=`basename $nistfolder`"-gcm"
    for keyfile in $nistfolder/keys/*
    do
        if [ $usingWSL -eq 0 ] ; then
            argkeyfile=$keyfile
        else
            argkeyfile=`wslpath -am "$keyfile"`
        fi
        testName=`basename $keyfile .xml`
        expectedResultsFile=$nistfolder/expected/$testName.pt.base64
        count=1

        while read expectedResult
        do
            paddedCount=`printf "%02d" $count`
            testFolderRoot=`basename $nistfolder`
            fileToTest=nist-aesgcm/decrypt/$testFolderRoot/files/$testName-$paddedCount
            
            expectedResult=$(echo $expectedResult | sed 's/\r$//')

            if [ -z $expectedResult ]; then
                execEncTest $res_fail \
                "" \
                "$fileToTest" \
                "$transformName" \
                "--keys-file $argkeyfile" \
                "" \
                ""
            else
                execEncTest $res_success \
            ...
            count=$(expr $count + 1)
        done < $expectedResultsFile
    done
done

execEncTest $res_success \
    "" \
    "nist-aesgcm/encrypt/aes128/enc-aes128gcm-keyname" \
    "aes128-gcm" \
    "" \
    "--keys-file $topfolder/nist-aesgcm/encrypt/keys/keys.xml --binary-data $topfolder/nist-aesgcm/encrypt/aes128/enc-aes128gcm-keyname.data" \
    "--keys-file $topfolder/nist-aesgcm/encrypt/keys/keys.xml"

execEncTest $res_success \
    "" \
    "nist-aesgcm/encrypt/aes192/enc-aes192gcm-keyname" \
    "aes192-gcm" \
    "" \
    "--keys-file $topfolder/nist-aesgcm/encrypt/keys/keys.xml --binary-data $topfolder/nist-aesgcm/encrypt/aes192/enc-aes192gcm-keyname.data" \
    "--keys-file $topfolder/nist-aesgcm/encrypt/keys/keys.xml"

execEncTest $res_success \
    "" \
    "nist-aesgcm/encrypt/aes256/enc-aes256gcm-keyname" \
    "aes256-gcm" \
    "" \
    "--keys-file $topfolder/nist-aesgcm/encrypt/keys/keys.xml --binary-data $topfolder/nist-aesgcm/encrypt/aes256/enc-aes256gcm-keyname.data" \
    "--keys-file $topfolder/nist-aesgcm/encrypt/keys/keys.xml"
