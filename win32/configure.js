...
function usage()
{
	...
	txt += "              with possible options: 'yes'/'leaks', 'asan', or 'no' (default).\n";
	...
	txt += "  apps:       Build binaries from the 'apps/' folder (" + (buildApps? "yes" : "no")  + ")\n";
	...
	txt += "              (" + buildSoPrefix + ")\n";
	txt += "  cruntime:   C runtime library to link against (" + cruntime + ")\n";
...
function discoverVersion()
{
	...
	vf.WriteLine("WITH_APPS=" + (buildApps ? "1" : "0"));
...
function configureXmlSecVersion()
{
	var fso, ofi, of, ln, s;
	fso = new ActiveXObject("Scripting.FileSystemObject");
	if (!fso.FileExists(versionHeaderIn)) {
		WScript.Echo("ERROR: " + versionHeaderIn + " not found.");
		error = 1;
		return;
	}
	...
	while (ofi.AtEndOfStream != true) {
		...
		if (s.search(/\@XMLSEC_VERSION_MAJOR\@/) != -1) {
			...
			of.WriteLine(s.replace(/\@XMLSEC_VERSION_INFO\@/,
				(parseInt(verMajorXmlSec) * 10000 + parseInt(verMinorXmlSec) * 100 + parseInt(verMicroXmlSec)) + ":0:0"));
		} else
			of.WriteLine(ln);
	}
	ofi.Close();
	...
	if (opt.length > 0) {
		if (opt == "crypto")
			...
			if (buildWithMemcheck == "") {
				WScript.Echo("ERROR: Invalid value for 'memcheck' parameter, supported options are 'yes'/'leaks', 'asan', or 'no'.\n");
				error = 1;
			...
			buildApps = strToBool(arg.substring(opt.length + 1, arg.length));
		else if (opt == "prefix")
...
for (j = 0; j < crlist.length; j++) {
	var cur = crlist[j].replace(/^\s+|\s+$/g, "");
	if (cur == "openssl") {
		curcrypto="openssl";
		withOpenSSL = 1;
		withOpenSSLVersion = "300"; /* default */
	} else if (cur == "openssl=300" || cur == "openssl-300") {
		curcrypto = "openssl";
		withOpenSSL = 1;
		withOpenSSLVersion = "300";
	} else if (cur == "openssl=111" || cur == "openssl-111") {
		curcrypto="openssl";
		withOpenSSL = 1;
		...
		WScript.Echo("Unknown crypto engine '" + cur + "' specified. Aborting.");
		WScript.Quit(1);
	}
	if (j == 0) {
...
txtOut += "Enable legacy features: " + boolToStr(withLegacyFeatures) + "\n";
...
txtOut += "\n";
txtOut += "\n";
txtOut += "DEPRECATED: the configure.js script is no longer supported and will be removed in a future release.\n";
